                                            ...
                                            <div class="options-review">
                                                @foreach($question->options as $option)
                                                    @php
                                                        ...
                                                        // DIPERBARUI: Logika pewarnaan yang lebih andal
                                                        if ($isStudentAnswer) {
                                                            // Jika ini adalah jawaban siswa...
                                                            $labelClass = $isQuestionCorrect ? 'bg-success text-white' : 'bg-danger text-white';
                                                        } elseif ($isCorrectAnswer) {
                                                            // Jika ini bukan jawaban siswa, tapi ini adalah kunci jawaban...
                                                            $labelClass = 'bg-info text-white';
                                                        }
                                                    @endphp
                                                    <div class="p-2 rounded mb-2 {{ $labelClass }}">
                                                        @if($isStudentAnswer && $isQuestionCorrect)
                                                            <i class="fa fa-check-circle-o mr-2"></i> <strong>Jawaban Anda (Benar)</strong>
                                                        @elseif($isStudentAnswer && !$isQuestionCorrect)
