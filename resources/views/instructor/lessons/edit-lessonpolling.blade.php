            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                            ...
                            <p class="m-b-10 fw-bolder" style="font-size: 2rem;">Edit Pelajaran Polling</p>
                ...
                <div class="col-md-12 d-flex mt-3">
                    <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="#!">Edit Polling</a></li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-block">
                        <form action="{{ route('instructor.lessons.update', $lesson) }}" method="POST" data-lesson-submit="true">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="polling_question" class="form-control" value="{{ $lesson->lessonable->question }}" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Deskripsi (Opsional)</label>
                                <div class="col-sm-10">
                                    <textarea name="polling_description" class="form-control" rows="4">{{ $lesson->lessonable->description }}</textarea>
                                </div>
                            </div>

                            <h5 class="mt-4">Opsi Polling</h5>
                            <div class="alert alert-warning">Mengubah opsi akan menghapus semua tanggapan siswa sebelumnya (karena data di-reset).</div>
                            <div id="polling-options-container">
                                @foreach($lesson->lessonable->options as $index => $option)
                                <div class="form-group row option-row">
                                    <label class="col-sm-2 col-form-label">Opsi {{ $index + 1 }}</label>
                                    <div class="col-sm-9">
                                        <input type="text" name="polling_options[]" class="form-control" value="{{ $option->text }}" required>
                                    </div>
                            ...
                            <div class="form-group row" id="max_choices_container" style="{{ optional($lesson->lessonable)->allow_multiple ? 'display: flex;' : 'display: none;' }}">
                                <label class="col-sm-2 col-form-label">Maksimal Pilihan</label>
                                <div class="col-sm-10">
                                    <input type="number" name="max_choices" class="form-control" min="2" placeholder="Biarkan kosong jika tidak ada batasan" value="{{ optional($lesson->lessonable)->max_choices }}">
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            @php
                                ...
                                $startTimeLocal = $lesson->lessonable->start_time ? Carbon\Carbon::parse($lesson->lessonable->start_time)->timezone($instructorTimezone)->format('Y-m-d\TH:i') : '';
                                $endTimeLocal = $lesson->lessonable->end_time ? Carbon\Carbon::parse($lesson->lessonable->end_time)->timezone($instructorTimezone)->format('Y-m-d\TH:i') : '';
                            @endphp
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Waktu Mulai</label>
                                <div class="col-sm-10">
                                    <input type="datetime-local" name="start_time" class="form-control" value="{{ $startTimeLocal }}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Waktu Selesai</label>
                                <div class="col-sm-10">
                                    <input type="datetime-local" name="end_time" class="form-control" value="{{ $endTimeLocal }}">
                                </div>
                            </div>
                            ...
                            <div class="form-group row mt-4">
                                <div class="col-sm-12 text-right">
                                    ...
                                    <button type="submit" class="btn btn-primary js-lesson-submit-btn">
                                        <span class="js-lesson-submit-text">Update Pelajaran</span>
