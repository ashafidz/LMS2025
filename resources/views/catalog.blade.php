...
@section('content')
            ...
            <form action="{{ route('courses') }}" method="GET">
                <div class="row justify-content-between align-items-center mb-4" data-aos="fade-up">
                    <div class="col-md-6">
                        ...
                        <p class="text-muted mb-0">Kami menyediakan kelas dengan materi yang dibuat sesuai kebutuhan di
                            dunia Profesional.</p>
                    </div>
                                ...
                                @foreach ($categories as $category)
                                    <div class="form-check mb-2">
                                        ...
                                        <label class="form-check-label"
                                            for="kategori-{{ $category->id }}">{{ $category->name }}</label>
                                    </div>
                                @endforeach
                                    ...
                                    <a href="{{ route('courses.show', $course->slug) }}"
                                        ...
                                        <div class="card h-100 shadow-sm border-0 rounded-4 course-card">
                                            ...
                                            <div class="card-body d-flex flex-column">
                                                <span
                                                    class="badge bg-primary-subtle text-primary-emphasis align-self-start mb-2">{{ $course->category->name }}</span>
                                                <h5 class="card-title fw-bold text-dark flex-grow-1">
                                                    {{ Str::limit($course->title, 50) }}</h5>
                                                <p class="card-text text-muted mb-2">{{ $course->instructor->name }}</p>

                                                @if ($course->reviews_count > 0)
                                                    <div class="d-flex align-items-center mb-2">
                                                        <span
                                                            class="fw-bold text-warning me-1 fs-5">{{ number_format($course->reviews_avg_rating, 1) }}</span>
                                                        <i class="bi bi-star-fill text-warning me-2"
                                                            style="font-size: 0.89rem;"></i>
                                            ...
                                            <div class="card-footer bg-white border-0 pt-0">
                                                ...
                                                @elseif($course->payment_type === 'diamonds')
                                                    <span class="fw-bold text-primary fs-5 d-flex align-items-center">
                                                        <i class="fa fa-diamond me-2"></i>
                                                        {{ number_format($course->diamond_price, 0, ',', '.') }} Diamonds
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                            ...
                            @empty
                                <div class="col-12">
                                    <div class="text-center p-5 bg-white rounded-4 shadow-sm">
                                        ...
                                        <p class="text-muted">Maaf, tidak ada kursus yang cocok dengan kriteria pencarian
                                            atau filter Anda.</p>
                                        <a href="{{ route('courses') }}" class="btn btn-primary mt-2">Hapus Filter & Lihat
                                            Semua</a>
                                    </div>
                                </div>
                            @endforelse
                                            ...
                                            <a href="{{ route('courses.show', $course->slug) }}"
                                                ...
                                                <div class="card h-100 shadow-sm border-0 rounded-4 course-card">
                                                    ...
                                                    <div class="card-body d-flex flex-column">
                                                        {{-- Kategori Kursus --}}
                                                        <span
                                                            class="badge bg-primary-subtle text-primary-emphasis align-self-start mb-2">
    ...
    <!-- Pricing Section -->
    <section id="pricing" class="pricing section " style="background-color: rgb(237, 248, 253);">
                        ...
                        <div class="pricing-card">
                            ...
                            <div class="price">
                                @if ($course->payment_type === 'money')
                                    ...
                                    @endif
                                @elseif($course->payment_type === 'diamonds')
                                    <span class="amount" style="font-size: 1.5rem;"><i class="fa fa-diamond mr-2"></i>
                                        {{ number_format($course->diamond_price, 0, ',', '.') }}</span>
                                @endif
                            </div>
